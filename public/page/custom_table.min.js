...
const TableHelper = {
    ...
    init: function (selector, options = {}) {
        const defaultConfig = {
            ...
            initComplete: function () {
                ...
                const keyMap = options.keyMap || {}; // <- tambahkan di sini agar fleksibel
                ...
                if (filterCols.length) {
                    api.columns(filterCols).every(function () {
                        ...
                        $.get(options.urlOpsiFilter, function (res) {
                            const headerKey = headerText.toLowerCase();
                            const mappedKey = keyMap[headerKey] || headerKey;
                            const values = res[mappedKey] || [];
    ...
    tabelTanggalWaktu: function (data, type) {
        ...
        if (!waktu) return formattedTanggal;
        return `${formattedTanggal} → ${waktu}`;
    },

    /**
     * Render kolom hitung mundur
     * Digunakan di konfigurasi kolom: render: TableHelper.tabelHitungMundur
     */
    tabelHitungMundur: function (data) {
        if (!data) return "-";
        const [awal, akhir] = data;
        const tglAwal = new Date(awal);
        const tglAkhir = new Date(akhir);
        const tglSekarang = new Date();
        const formattedHitungMundur = FormatHelper.formatHitungMundur(tglSekarang, tglAwal, tglAkhir)
        return formattedHitungMundur;
    },
};
